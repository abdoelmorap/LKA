...
class ChatLoadMore extends LoadingMoreBase<ChatMessage?> {
    ...
    try {
      ...
      if (this.length == 0) {
        result = await _dio
          ...
          options: Options(
            headers:
                Utils.setHeader(chatOpenController!.token.value.toString()),
          ),
        )
            .catchError((onError) {
          isSuccess = false;
      ...
      if (result != null && result.statusCode == 200) {
        ...
        if (source.messages != null && source.messages.length != 0) {
          print(source);
          FLog.warning(text: source.messages.length);
          if (source.messages.length != 1) {
  ...
  Future checkNewMsg() async {
    ...
    try {
      ...
      FLog.debug(text: 'RESPONSE CHECK', exception: msgCheckResultData);
      ...
      if (response.statusCode == 200) {
        ...
        FLog.warning(text: data.toString());
  ...
  Future submitText(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        FLog.warning(text: "Lenght -> ${this.length}");
        if (_chatController.chatSettings.value.chatSettings!.chatMethod !=
            'pusher') {
